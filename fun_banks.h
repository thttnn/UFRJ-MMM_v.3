...
*/
	...
	if(v[1]+v[4]+v[10]!=0)//no regulatory rule and no sensitivity
...
v[2]=V("Bank_Profits");
v[3]=V("Bank_Distributed_Profits");
v[4]=v[1]-v[2]+v[0]-v[3];
RESULT(v[4])
...
EQUATION("Bank_Exit")
/*
A Bank exits if accumulated capital goes to zero. 
Government has to pay for bank losses.
That bank's clients are redistributed to other banks proportionally to market shares.
*/
v[0]=V("Bank_Accumulated_Profits");
v[16]=COUNTS(PARENT, "BANKS");
if(v[0]<=0&&v[16]<0)
	{
	WRITE("Bank_Market_Share", 0);
	WRITE("Bank_Accumulated_Profits", 0);
	v[1]=SUMS(PARENT, "Bank_Market_Share");												
	CYCLES(PARENT, cur, "BANKS")															
	{
	v[2]=VS(cur,"Bank_Market_Share");
		if(v[2]!=0)
		WRITES(cur, "Bank_Market_Share", (v[2]/v[1]));							
	}
	
	v[3]=V("Bank_Stock_Loans_Long_Term");
	v[4]=V("Bank_Stock_Loans_Short_Term");
	v[5]=V("Bank_Total_Stock_Loans");
	v[6]=V("Bank_Stock_Deposits");
	
	CYCLES(PARENT, cur, "BANKS")															
	{
	v[7]=VS(cur,"Bank_Market_Share");
		if(v[7]!=0)
		{
		v[8]=VS(cur,"Bank_Stock_Loans_Long_Term");
		v[9]=VS(cur, "Bank_Stock_Loans_Short_Term");
		v[10]=VS(cur, "Bank_Total_Stock_Loans");
		v[11]=VS(cur, "Bank_Stock_Deposits");	
		WRITES(cur, "Bank_Stock_Loans_Long_Term", (v[8]+v[7]*v[3]));
		WRITES(cur, "Bank_Stock_Loans_Short_Term", (v[9]+v[7]*v[4]));
		WRITES(cur, "Bank_Total_Stock_Loans", (v[10]+v[7]*v[5]));
		WRITES(cur, "Bank_Stock_Deposits", (v[11]+v[7]*v[6]));
		}
	}
	...
	CYCLES(root, cur, "SECTORS")
	{
		CYCLES(cur, cur1, "FIRMS")
		{
			v[13]=VS(cur1, "id_firm_bank");
			if(v[13]==v[12])
			{
				cur2=RNDDRAWS(root, "BANKS", "Bank_Market_Share");
				v[14]=VS(cur2, "id_bank");
				WRITES(cur1, "id_firm_bank", v[14]);
			}
		}
	}
	v[15]=VS(PARENT, "financial_sector_accumulated_losses");
	WRITES(PARENT,"financial_sector_accumulated_losses", (v[15]-v[0]));
	DELETE(p);
	}
RESULT(0)
