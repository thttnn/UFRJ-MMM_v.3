...
folder    <- "./Res_Interest"                  # data files folder
...
expVal <- c("Baseline", "WeakShort", "StrongShort", "WeakLong", "StrongLong", "WeakBoth", "StrongBoth")                            # case parameter values
...
source( "time-plots-finance-2.R" )
source( "box-plots-finance-2.R" )
...
tryCatch({    # enter error handling mode so PDF can be closed in case of error/interruption
  ...
  for(k in 1 : nExp){ # Experiment k
    ...
    plot_bpf( list( log0( Adata[[ k ]]$Real_GDP ), log0( Adata[[ k ]]$DEBT_FS ), 
                    log0( Adata[[ k ]]$DEP_FS ) ),
              ...
              leg = c( "GDP", "Stock of Loans", "Stock of Deposits" ),
    ...
    key.stats.1 <- matrix(
      c(
        ...
        ## (s.e.)
        sd( debtfsst_gr ) / sqrt( nSize ), 
        sd( debtfslt_gr ) / sqrt( nSize ),
        ...
        ## ADF test (logs) s.e.
        sd( unname( sapply( adf_dst_g, `[[`, "statistic" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_dlt_g, `[[`, "statistic" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_d_g, `[[`, "statistic" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_dep_g, `[[`, "statistic" ) ) ) / sqrt( nSize ),
        ...
        sd( unname( sapply( adf_dst_g, `[[`, "p.value" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_dlt_g, `[[`, "p.value" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_d_g, `[[`, "p.value" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_dep_g, `[[`, "p.value" ) ) ) / sqrt( nSize ),
        ...
        se( debtfs_st_sd ) / sqrt( nSize ), 
        se( debtfs_lt_sd ) / sqrt( nSize ), 
    ...
    colnames( key.stats.1 ) <- c( "Short Term Loans", 
                                  "Long Term Loans", 
                                  "Total Loans", 
                                  "Total Deposits"
                                  )
    ...
    key.stats.2 <- matrix(
      c(
        ...
        ## (s.e.)
        sd( fs_hhi  ) / sqrt( nSize ), 
        sd( fs_lev  ) / sqrt( nSize ), 
        sd( fs_str  ) / sqrt( nSize ),
        sd( fs_dr  ) / sqrt( nSize ),
        sd( ponzi ) / sqrt( nSize ),
        sd( spec ) / sqrt( nSize ), 
        sd( hedge ) / sqrt( nSize ),
        ...
        ## ADF test (logs) s.e.
        sd( unname( sapply( adf_fshhi, `[[`, "statistic" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_fslev, `[[`, "statistic" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_fsstr, `[[`, "statistic" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_fsdr, `[[`, "statistic" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_ponzi, `[[`, "statistic" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_spec, `[[`, "statistic" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_hedge, `[[`, "statistic" ) ) ) / sqrt( nSize ),
        ...
        ## ADF test (logs) p.value s.e.
        sd( unname( sapply( adf_fshhi, `[[`, "p.value" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_fslev, `[[`, "p.value" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_fsstr, `[[`, "p.value" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_fsdr, `[[`, "p.value" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_ponzi, `[[`, "p.value" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_spec, `[[`, "p.value" ) ) ) / sqrt( nSize ),
        sd( unname( sapply( adf_hedge, `[[`, "p.value" ) ) ) / sqrt( nSize ),
        ...
        ## S.e. of bpf series s.d.
        se( fshhi_sd ) / sqrt( nSize ),
        se( fslev_sd ) / sqrt( nSize ), 
        se( fsstr_sd ) / sqrt( nSize ),
        se( fsdr_sd ) / sqrt( nSize ),
        se( ponzi_sd ) / sqrt( nSize ),
        se( spec_sd ) / sqrt( nSize ), 
        se( hedge_sd ) / sqrt( nSize ), 
    ...
    colnames( key.stats.2 ) <- c( "Financial Sector I.HHI", 
    ...
    corr.struct.2 <- matrix(c(colMeans(t( unname( sapply(fspr_gdp, `[[`, "acf" ) ) ), na.rm = T),
                              colSds(t( unname( sapply(fspr_gdp, `[[`, "acf" ) ) ), na.rm = T) / sqrt( nSize ),
                              fspr_gdp_pval,
                              
                              colMeans(t( unname( sapply(fsdr_gdp, `[[`, "acf" ) ) ), na.rm = T),
                              colSds(t( unname( sapply(fsdr_gdp, `[[`, "acf" ) ) ), na.rm = T) / sqrt( nSize ),
                              fsdr_gdp_pval,
                              
                              colMeans(t( unname( sapply(ponzi_gdp, `[[`, "acf" ) ) ), na.rm = T),
                              colSds(t( unname( sapply(ponzi_gdp, `[[`, "acf" ) ) ), na.rm = T) / sqrt( nSize ),
                              ponzi_gdp_pval,
                              
                              colMeans(t( unname( sapply(spec_gdp, `[[`, "acf" ) ) ), na.rm = T),
                              colSds(t( unname( sapply(spec_gdp, `[[`, "acf" ) ) ), na.rm = T) / sqrt( nSize ),
                              spec_gdp_pval,
                              
                              colMeans(t( unname( sapply(hedge_gdp, `[[`, "acf" ) ) ), na.rm = T),
                              colSds(t( unname( sapply(hedge_gdp, `[[`, "acf" ) ) ), na.rm = T) / sqrt( nSize ),
                              hedge_gdp_pval
    ...
    rownames( corr.struct.2 ) <- c( "Financial Sector Profits", " (s.e.)", " (p-val.)",
                                    "Financial Sector Default Rate", " (s.e.)", " (p-val.)",
                                    "Share of Ponzi Firms", " (s.e.)", " (p-val.)",
                                    "Share of Speculative Firms", " (s.e.)", " (p-val.)",
                                    "Share of Hedge Firms", " (s.e.)", " (p-val.)"
    )
